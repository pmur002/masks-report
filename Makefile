...
TARFILE = ../masks-deposit-$(shell date +'%Y-%m-%d').tar.gz
...
docker:
	sudo docker build -t pmur002/masks-report .
	sudo docker run -v $(shell pwd):/home/work/ -w /home/work --rm pmur002/masks-report make masks.html

web:
	make docker
	cp -r ../masks-report/* ~/Web/Reports/GraphicsEngine/masks/
